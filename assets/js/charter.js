...
(function ($) {
    ...
    $(document).on('click', '.apc-calc-submit', async function () {
        ...
        try {
            ...
            $('#apc-results-panel').slideDown(300);
            $('html,body').animate({
    ...
    function renderResults($widget, f, p, meta) {
        ...
        // ── Distance ──────────────────────────────────
        // airport.airway_distance       = actual route (nm shown as km on AV site)
        // airport.great_circle_distance = straight-line
        const airwayDist = airportObj.airway_distance;
        const gcDist     = airportObj.great_circle_distance;

        // AviaPages shows these in km on their site
        $('#res-distance').text(airwayDist != null ? airwayDist + ' km' : '—');
        $('#res-gc-distance').text(gcDist != null && gcDist !== airwayDist
            ? 'GC: ' + gcDist + ' km' : '');
        ...
        if (Array.isArray(fuelDetailed) && fuelDetailed.length) {
            ...
            $('#res-fuel-detail').hide(); // collapsed by default
            $('#res-fuel-toggle').show(); // show the toggle button
        } else {
    ...
    $(document).on('click', '#res-fuel-toggle', function () {
        ...
        if ($detail.is(':visible')) {
            $detail.slideUp(200);
            $(this).text('▼ Show Fuel Breakdown by Phase');
        } else {
            $detail.slideDown(200);
            $(this).text('▲ Hide Fuel Breakdown');
        }
    });
